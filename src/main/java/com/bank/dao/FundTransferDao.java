...
public class FundTransferDao  {
	...
	public synchronized boolean confirmTransaction(Transaction tr) {
		...
		// String getPayeeQuery="select * from gr13_payee where GP_GC_CUSTOMER_ACCOUNT_NUMBER="+tr.getFrom_account(); 

		//UPDATE BALANCE OF CUSTOMER AND PAYEE
		String updateCustomerBalance = "update gr13_accounts set ga_balance=ga_balance-"+(tr.getAmount()+tr.getCharges())+" where ga_account_number="+tr.getFrom_account();
		i = jdbcTemplate.update(updateCustomerBalance);

		String updatePayeeBalance = "update gr13_accounts set ga_balance=ga_balance+"+tr.getAmount()+" where ga_account_number="+tr.getTo_account();
